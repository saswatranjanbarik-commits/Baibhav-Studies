...
export default function Calendar() {
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(<div key={`empty-${i}`} className="min-h-[100px] sm:min-h-[120px] p-2 bg-slate-50/50 border border-slate-100"></div>);
    }
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        <div 
          key={d} 
          className={`min-h-[100px] sm:min-h-[120px] p-2 border ${isToday ? 'bg-indigo-50/30 border-indigo-200' : 'bg-white border-slate-200'} transition-colors hover:bg-slate-50 relative group flex flex-col`}
          onClick={() => {
